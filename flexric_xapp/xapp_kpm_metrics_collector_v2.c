 ...
 * 
 * Metrics: CQI, SNR (proxy for RSRP), BLER, MCS, TBS, PRB, Throughput, Buffer stats
 * 
...
static void write_csv_row(void) {
    ...
    if (sample_count % PRINT_INTERVAL == 0) {
        printf("[%lu] SNR=%.1fdB BLER=%.3f MCS=%u DL_Thp=%.1fkbps UL_Thp=%.1fkbps PRB=%u/%u\n",
               sample_count, metrics.pusch_snr, metrics.dl_bler, metrics.dl_mcs1,
               metrics.dl_thp_kbps, metrics.ul_thp_kbps, 
               metrics.dl_prb, metrics.ul_prb);
        fflush(csv_file);
    }
...
static void sm_cb_kpm(sm_ag_if_rd_t const* rd) {
    ...
    if (msg_frm_3->ue_meas_report_lst_len == 0) return;
    ...
    for (size_t i = 0; i < msg_frm_3->ue_meas_report_lst_len; i++) {
        ...
        for (size_t j = 0; j < msg_frm_1->meas_data_lst_len; j++) {
            for (size_t z = 0; z < msg_frm_1->meas_data_lst[j].meas_record_len; z++) {
                if (msg_frm_1->meas_info_lst_len == 0) continue;
                if (msg_frm_1->meas_info_lst[z].meas_type.type != NAME_MEAS_TYPE) continue;
                ...
                // Extract values
                meas_record_lst_t const* rec = &msg_frm_1->meas_data_lst[j].meas_record_lst[z];
...
static kpm_act_def_t gen_kpm_act_def(void) {
    ...
    act_def.frm_4.action_def_format_1.meas_info_lst = calloc(num_meas, sizeof(meas_info_format_1_lst_t));
    
    for (size_t i = 0; i < num_meas; i++) {
        act_def.frm_4.action_def_format_1.meas_info_lst[i] = gen_meas_info(meas_names[i]);
    }
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < (int)nodes.len; i++) {
        if (mac_h[i].success) rm_report_sm_xapp_api(mac_h[i].u.handle);
        if (rlc_h[i].success) rm_report_sm_xapp_api(rlc_h[i].u.handle);
        if (pdcp_h[i].success) rm_report_sm_xapp_api(pdcp_h[i].u.handle);
        if (gtp_h[i].success) rm_report_sm_xapp_api(gtp_h[i].u.handle);
        if (kpm_h[i].success) rm_report_sm_xapp_api(kpm_h[i].u.handle);
    }
    
    free(mac_h); free(rlc_h); free(pdcp_h); free(gtp_h); free(kpm_h);
