...
During the deployment phase, a critical configuration issue affecting the 5G Core Network was identified and resolved. The \texttt{oai-core} database (MySQL) contained incorrect IMSI (International Mobile Subscriber Identity) entries in its provisioning tables. This mismatch prevented the simulated User Equipment (UE) from successfully authenticating and registering with the core. The issue was resolved by correcting the SQL initialization scripts to match the specific IMSQs and security keys used by the simulated UE, ensuring successful attachment.
